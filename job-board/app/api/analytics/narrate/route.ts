...
export async function POST(req: NextRequest) {
  const limited = rateLimit(req, "narrate", 10, 60_000);
...
Week-over-week change: ${
      prev7 > 0
        ? `${Math.round(((trend7 - prev7) / prev7) * 100)}%`
        : "N/A (no prior data)"
    }

Write a CONCISE 2-sentence headline + summary for developers following Arc ecosystem activity. Do not use em dashes or en dashes.
...
{"headline":"max 10 words","summary":"2-sentence summary","trend":"up"|"down"|"neutral"}`;
    ...
    console.error("Analytics narration error:", err);
    const e = err as { status?: number; message?: string };
    const providerIssue =
      typeof e?.status === "number" ||
      /gemini|openrouter|quota|api key|credit|rate limit|overloaded|unavailable/i.test(
        e?.message ?? ""
      );
    return NextResponse.json(
      {
