...
export async function arcGetBalance(args: z.infer<typeof getBalanceSchema>) {
...
export async function arcGetBlock(args: z.infer<typeof getBlockSchema>) {
  const client = getPublicClient();
...
export async function arcGetTransaction(
  args: z.infer<typeof getTransactionSchema>
) {
  const client = getPublicClient();

  const [tx, receipt] = await Promise.all([
    client.getTransaction({ hash: args.hash as `0x${string}` }),
    client
      .getTransactionReceipt({ hash: args.hash as `0x${string}` })
      .catch(() => null),
  ]);
...
export async function arcGetGasPrice() {
  ...
  return {
    ...
    recommended: `${formatGwei(gasPrice + 5_000_000_000n)} Gwei`,
    history: feeHistory
      ? {
          blocks: feeHistory.baseFeePerGas?.map((fee) =>
            fee ? `${formatGwei(fee)} Gwei` : null
          ),
        }
      : null,
