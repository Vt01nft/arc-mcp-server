...
export async function arcCreateJob(args: z.infer<typeof createJobSchema>) {
  ...
  if (!isAddress(args.provider)) throw new Error(`Invalid provider: ${args.provider}`);
...
export async function arcGetJob(args: z.infer<typeof getJobSchema>) {
  const client = getPublicClient();
...
export async function arcSubmitDeliverable(
  args: z.infer<typeof submitDeliverableSchema>
) {
  const client = getPublicClient();
  const { client: walletClient, account } = getWalletClient();
