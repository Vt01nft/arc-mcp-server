...
const TOOLS = [
    // ── Chain / Balance ──────────────────────────────────────────────────────
    {
        ...
        description: "Get USDC balance for any Arc Testnet address. Returns both the ERC-20 transfer balance (6 decimals) and the native gas balance (18 decimals).",
        inputSchema: toJsonSchema(getBalanceSchema),
    },
    {
        name: "arc_get_block",
        description: "Get Arc Testnet block info. Omit blockNumber for the latest block. Returns number, hash, timestamp, tx count, gas used.",
        inputSchema: toJsonSchema(getBlockSchema),
    },
    {
        name: "arc_get_transaction",
        description: "Get Arc Testnet transaction details by hash. Returns status, from/to, value, gas used, block number.",
        inputSchema: toJsonSchema(getTransactionSchema),
    },
    {
        name: "arc_get_gas_price",
        description: "Get the current USDC gas price on Arc Testnet. Minimum is 20 Gwei on testnet. Returns current price and recommended max fee.",
        inputSchema: { type: "object", properties: {}, required: [] },
    },
    // ── USDC Transfers ───────────────────────────────────────────────────────
    {
        name: "arc_send_usdc",
        description: "Send USDC to another address on Arc Testnet. Requires PRIVATE_KEY env var. Set simulate=true to estimate gas without broadcasting.",
        inputSchema: toJsonSchema(sendUsdcSchema),
    },
    {
        name: "arc_approve_usdc",
        description: "Approve a contract (like ERC-8183) to spend USDC. Required before funding jobs. Use amount='unlimited' for max approval.",
        inputSchema: toJsonSchema(approveUsdcSchema),
    },
    // ── ERC-8183 Jobs ────────────────────────────────────────────────────────
    {
        name: "arc_get_job_count",
        description: "Get total number of ERC-8183 jobs created on Arc Testnet. Useful for monitoring ecosystem activity.",
        inputSchema: { type: "object", properties: {}, required: [] },
    },
    {
        name: "arc_get_job",
        description: "Get ERC-8183 job details by ID. Returns client, provider, evaluator, status (Open/Funded/Submitted/Completed/Rejected/Expired), USDC amount, description hash, deliverable hash, and expiry.",
    ...
    },
    {
        ...
        description: "Provider submits deliverable for an ERC-8183 job. Pass the work content or IPFS URI - it will be hashed to bytes32 onchain. Requires PRIVATE_KEY.",
        inputSchema: toJsonSchema(submitDeliverableSchema),
    },
    {
        name: "arc_complete_job",
        description: "Evaluator approves deliverable and releases USDC to provider. Finalizes the ERC-8183 job. Requires PRIVATE_KEY (evaluator wallet).",
        inputSchema: toJsonSchema(completeJobSchema),
    },
    {
        name: "arc_reject_job",
        description: "Evaluator rejects deliverable and refunds USDC to client. Requires PRIVATE_KEY (evaluator wallet). Provide a reason.",
        inputSchema: toJsonSchema(rejectJobSchema),
    },
    {
        name: "arc_refund_job",
        description: "Trigger a refund for an expired ERC-8183 job. USDC returns to client. Job must be past its expiry timestamp. Requires PRIVATE_KEY.",
        inputSchema: toJsonSchema(refundJobSchema),
    },
    // ── ERC-8004 Agents ──────────────────────────────────────────────────────
    {
        name: "arc_give_reputation",
        description: "Record reputation feedback for an ERC-8004 agent. Score: -100 to 100. Cannot be called by the agent's own owner. Requires PRIVATE_KEY (validator wallet).",
        inputSchema: toJsonSchema(giveReputationSchema),
    },
    {
        name: "arc_get_reputation",
        description: "Get ERC-8004 reputation score for an agent. Returns total score, event count, and average score.",
        inputSchema: toJsonSchema(getReputationSchema),
    },
    {
        name: "arc_request_validation",
        description: "Request ERC-8004 validation for an agent from a specific validator. Pass a URI pointing to KYC/credential data. Requires PRIVATE_KEY (agent owner wallet).",
        inputSchema: toJsonSchema(requestValidationSchema),
    },
    {
        name: "arc_respond_validation",
        description: "Validator responds to an ERC-8004 validation request. passed=true records result 100 (passed), false records 0 (failed). Requires PRIVATE_KEY (validator wallet).",
        inputSchema: toJsonSchema(respondValidationSchema),
    },
    {
        name: "arc_get_validation",
        description: "Get ERC-8004 validation result for an agent. Returns validator, result (PASSED/FAILED/PENDING), and URIs.",
        inputSchema: toJsonSchema(getValidationSchema),
    },
    // ── Events ───────────────────────────────────────────────────────────────
    {
        name: "arc_get_events",
        description: "Fetch raw contract events from Arc Testnet. Choose contract: 'jobs' (ERC-8183), 'reputation' (ERC-8004), or 'usdc'. Scans last 1000 blocks by default.",
        inputSchema: toJsonSchema(getEventsSchema),
    },
    {
        name: "arc_get_job_events",
        description: "Get the full event timeline for a specific ERC-8183 job - creation, funding, submission, completion/rejection in chronological order.",
...
function toJsonSchema(schema) {
    ...
    for (const [key, field] of Object.entries(shape)) {
        const f = field;
        const def = f._def;
        const isOptional = f instanceof z.ZodOptional ||
        ...
        const inner = f instanceof z.ZodOptional || f instanceof z.ZodDefault
            ? f._def.innerType ?? f._def.schema
            : f;
