import { z } from "zod";
import { getPublicClient } from "../arc-client.js";
import { ADDRESSES } from "../contracts/addresses.js";
import { ERC8183_ABI, ERC8004_REPUTATION_ABI, USDC_ABI } from "../contracts/abis.js";
// ── arc_get_events ─────────────────────────────────────────────────────────────
export const getEventsSchema = z.object({
    ...
    event: z
        .string()
        .optional()
        .describe("Specific event name, e.g. 'JobCreated', 'JobCompleted', 'FeedbackGiven', 'Transfer'. " +
...
export async function arcGetEvents(args) {
    ...
    return {
        ...
        events: parsed,
        note: "Raw log data is returned. Use viem's decodeEventLog with the contract ABI for structured output.",
    };
}
// ── arc_get_job_events ─────────────────────────────────────────────────────────
...
export async function arcGetJobEvents(args) {
    const client = getPublicClient();
    const latestBlock = await client.getBlockNumber();
    const fromBlock = 1n; // scan full history for the job
    const jobIdHex = ("0x" + args.jobId.toString(16).padStart(64, "0"));
    ...
    const [created, funded, submitted, completed, rejected] = await Promise.all([
        client.getLogs({
            ...
            event: ERC8183_ABI.find((x) => x.type === "event" && x.name === "JobCreated"),
        ...
        client.getLogs({
            address: ADDRESSES.ERC8183_JOB,
            event: ERC8183_ABI.find((x) => x.type === "event" && x.name === "JobFunded"),
            args: { jobId: BigInt(args.jobId) },
            fromBlock,
            toBlock: latestBlock,
        }).catch(() => []),
        ...
        client.getLogs({
            address: ADDRESSES.ERC8183_JOB,
            event: ERC8183_ABI.find((x) => x.type === "event" && x.name === "JobCompleted"),
            args: { jobId: BigInt(args.jobId) },
            fromBlock,
            toBlock: latestBlock,
        }).catch(() => []),
        client.getLogs({
            address: ADDRESSES.ERC8183_JOB,
            event: ERC8183_ABI.find((x) => x.type === "event" && x.name === "JobRejected"),
            args: { jobId: BigInt(args.jobId) },
            fromBlock,
            toBlock: latestBlock,
        }).catch(() => []),
