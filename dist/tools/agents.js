...
import { getPublicClient, getWalletClient, txLink, } from "../arc-client.js";
...
export async function arcGiveReputation(args) {
...
export async function arcGetReputation(args) {
    const client = getPublicClient();
    ...
    const avgScore = eventCount > 0n ? Number(totalScore) / Number(eventCount) : null;
    return {
        agentId: args.agentId,
...
export const requestValidationSchema = z.object({
    ...
    requestURI: z
        .string()
        .describe('URI pointing to validation request data, e.g. "ipfs://bafkrei..." or "https://..."'),
});
export async function arcRequestValidation(args) {
    const client = getPublicClient();
    const { client: walletClient, account } = getWalletClient();
    if (!isAddress(args.validator)) {
        throw new Error(`Invalid validator address: ${args.validator}`);
    }
    const requestHash = keccak256(toHex(`validation_request_agent_${args.agentId}_${Date.now()}`));
    const hash = await walletClient.writeContract({
    ...
    return {
        ...
        requestHash,
        next_step: "The validator must now call arc_respond_validation with their decision (100=pass, 0=fail).",
    };
}
...
export async function arcRespondValidation(args) {
    const client = getPublicClient();
    const { client: walletClient, account } = getWalletClient();
    const result = args.passed ? 100 : 0;
    const responseHash = keccak256(toHex(`validation_response_${args.agentId}_${args.requestId}_${result}`));
    const hash = await walletClient.writeContract({
        address: ADDRESSES.ERC8004_VALIDATION,
        abi: ERC8004_VALIDATION_ABI,
...
export async function arcGetValidation(args) {
    const client = getPublicClient();
    const result = await client.readContract({
        address: ADDRESSES.ERC8004_VALIDATION,
        abi: ERC8004_VALIDATION_ABI,
